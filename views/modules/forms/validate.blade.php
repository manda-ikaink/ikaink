{{-- Form template will apply bootstrap validation styles to invalid fields on submit --}}
@if(isset($form))
  {!! Form::open(['route' => ['forms.response.store'], 'files' => true, 'class' => 'fusion-form needs-validation', 'novalidate']) !!}
    <div class="row"> 
      @foreach($fieldGroups as $group => $fields)
        @foreach($fields as $field)
          @include('fields.default.' . $field->type, ['field' => $field, 'value' => $field->value,  'validate' => true, 'class' => 'col-12'])
        @endforeach
      @endforeach
