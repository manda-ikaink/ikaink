...
<div class="page-images container text-center wysiwyg mb-5">
    <figure class="page-images__figure d-flex flex-wrap justify-content-center">
        ...
        <picture class="page-images__image">
            <source srcset="{{ theme(theme_property('settings.images.loading')) }}" data-srcset="{{ $image->path() }}?w=500&amp;fit=max" media="--xs" />
            <source srcset="{{ theme(theme_property('settings.images.loading')) }}" data-srcset="{{ $image->path() }}?w=700&amp;fit=max" media="--sm" />
